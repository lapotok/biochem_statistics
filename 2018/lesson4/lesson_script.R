...
g3 = iris %>% 
  gghistogram("Petal.Length", y = "..density..", alpha=.3, col=NA, fill="Species", add_density = T, legend="right", rug = T)
g3
...
# превращаем концентрации в проценты относительно группы контроля
...
# влияние поправки на множественность сравнений: без vs Бонферрони
pr_st %$% pairwise.t.test(ConcUndilutedP, Sample, p.adjust.method = "none")
pr_st %$% pairwise.t.test(ConcUndilutedP, Sample, p.adjust.method = "bonferroni")

# Тест Тьюки
pr_st_aov %>% TukeyHSD
pr_st_aov %>% TukeyHSD %>% plot

# используем пайплайн, чтобы причесать таблицу вывода TukeyHSD
pr_st_aov %>% 
  TukeyHSD %>% 
  pluck('Sample') %>% 
  as.data.frame %>% 
