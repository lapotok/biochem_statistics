...
**Подсказка 1**: для сохранения результатов вычислений на каждом этапе удобно сначала создать пустой вектор командой `my_vec=c()`, а потом пополнять его командой `my_vec = c(my_vec, new_element)`.
