...
Повторять сказанное на лекции полезно, но не главное! Главное запоминать принцип и учиться гуглить. В этом задании мы продвинемся немного дальше в плане автоматизации и освоим функцию `aggregate()`. Почитайте справку по ней, погуглите примеры ее использования.
Задание будет заключаться в том, чтобы посчитать одной командой среднее каждого признака для каждого вида ириса. Чтобы можно было применить функцию `aggregate()`, сначала Вам надо будет преобразовать таблицу iris в "длинный" формат (группирующая переменная - _Species_) командой `melt()` из пакета `reshape2` (см. примеры длинного и широкого форматов в конспекте). Приведите результаты и код.
